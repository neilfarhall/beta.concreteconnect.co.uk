...
/**
 * Make link between Drupal content and file content.
 *
 * @package Drupal\exif
 */
class ExifContent {
  ...
  public function checkTitle($entityType, FieldableEntityInterface $entity) {
    ...
    if (in_array($entity->bundle(), $bundles_to_check)) {
      ...
      $exif_fields = $this->filterFieldsOnSettings($entityType, $entity);
      $exif_fields = $exif->getMetadataFields($exif_fields);
      foreach ($exif_fields as $drupal_field => $metadata_field_descriptor) {
   ...
   */
  public function entityInsertUpdate($entityType, FieldableEntityInterface $entity, $update = TRUE) {
    $bundles_to_check = $this->getBundleForExifData();
    if (in_array($entity->bundle(), $bundles_to_check)) {
      $exif = ExifFactory::getExifInterface();
      $exif_fields = $this->filterFieldsOnSettings($entityType, $entity);
      ...
      if (!$update && isset($entity->original)) {
        ...
        $metadata_images_fields = $this->getImageFieldsMetadata($entity, $exif_fields, $image_fields);
        foreach ($exif_fields as $drupal_field => $metadata_field_descriptor) {
          $field_name = $drupal_field;
          $field = $entity->get($field_name);
   ...
   *   The fields which contain Exif data.
   * @param array $image_fields
   *   Fields which contain images.
   *
   * @return array|bool
   *   Returns a boolean.
   *
   ...
   */
  private function getImageFieldsMetadata(FieldableEntityInterface $entity, array &$exif_fields, array $image_fields) {
    $result = [];
    ...
    foreach ($exif_fields as $drupal_field => $metadata_settings) {
      ...
      if (empty($image_fields[$field_image_name])) {
        ...
        $exif_fields[$drupal_field]['language'] = $language;
      }
    }
    return $result;
  }
  ...
  private function sanitizeValue($exif_value) {
    if (!Unicode::validateUtf8($exif_value)) {
      $utf8_exif_value = mb_convert_encoding($exif_value, 'UTF-8', 'ISO-8859-1');
      $exif_value = Html::escape($utf8_exif_value);
    }
    return (string) $exif_value;
  }
